...
raw_data = r"E:\#InclusionProject\Completed_City"  # provide ULA output folder path, Note Data folder str: Output\CITYNAME folder
#raw_data = r"E:\#InclusionProject\Output" # for error city
...
for current_data_dir in os.listdir(raw_data):
    ...
    ####
    # reclassifying as per new raster rule like: existing built-up = inclusion + existing

    New_RUL_RemapVal = arcpy.sa.RemapValue([[1,1],[2,2],[3,2],[4,2],[5,1]])

    output_file_t1 = r"%s\%s_t1_mosiac.img" % (reclassWS,dirname)
    output_file_t2 = r"%s\%s_t2_mosiac.img" % (reclassWS,dirname)

    #t1_new_mosiac = 

    mos_t1 = arcpy.sa.Reclassify(output_file_t1, "VALUE",New_RUL_RemapVal, "NODATA")
    mos_t1_file = mos_t1.save(r"%s\%s_t1_mosiac_new.img" % (reclassWS,dirname))

    mos_t2 = arcpy.sa.Reclassify(output_file_t2, "VALUE",New_RUL_RemapVal, "NODATA")
    mos_t1_file = mos_t2.save(r"%s\%s_t2_mosiac_new.img" % (reclassWS,dirname))

    

    invalueRaster_t1 = r"%s\%s_t1_mosiac_new.img" % (reclassWS,dirname)
    invalueRaster_t2 = r"%s\%s_t2_mosiac_new.img" % (reclassWS,dirname)
    ...
    outjoin_t1 = r"%s\%s_t1_join_new.shp" % (centriod_data,dirname)
    ...
    outjoin_t2 = r"%s\%s_t2_join_new.shp" % (centriod_data,dirname)
